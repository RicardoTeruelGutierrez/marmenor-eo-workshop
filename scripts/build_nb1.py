...
| **C — Instructor figures (real GeoTIFFs)** | True-Colour / NDCI / zone maps from real Sentinel-2 scenes downloaded with `scripts/download_sentinel2.py`. | Yes (one-off download) |
...
A free CDSE account (https://dataspace.copernicus.eu, no credit card) is only needed for Section B. Sections A and C run from files already in `data/`.
""")
...
The next cell self-bootstraps. In Colab it clones the workshop repo, installs dependencies and generates the synthetic datasets (~30 s, one time). On a local install it's a no-op — your existing environment keeps working.
""")
