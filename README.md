...
**Language:** English
**Format:** Hands-on Jupyter notebooks (Google Colab — no install)

---

## ▶️ The session — start here (60 min)

The main deliverable is a single, self-contained **1-hour live session**: you
download real satellite and pollution data with your own hands and watch the 2021
Mar Menor algal-bloom appear on screen. No install, mostly no account.
...
&nbsp; **← click, then Runtime → Run all**

- **Notebook:** [`notebooks/00_live_session.ipynb`](notebooks/00_live_session.ipynb)
- **Slides:** [`slides/session.html`](slides/session.html) (open in a browser)
- **Covers:** browse with NASA GIBS → download real Sentinel-2 (the COG trick) → measure the bloom with NDCI → live air quality → *(optional)* your own Copernicus key for on-demand products.

### 60-minute agenda

| Time | Step | You get |
|------|------|---------|
| 0:00 | Intro + setup (`Run all`) | Everything installed (~30 s) |
| 0:05 | 1 · Browse with NASA GIBS | A true-colour image of any date |
| 0:13 | 2 · Download real Sentinel-2 | Only the pixels over the bay (~16 MB) |
| 0:25 | 3 · Measure the bloom (NDCI) | The chlorophyll map of the 2021 crisis |
| 0:37 | 4 · Live air quality | A 7-day PM2.5 / ozone series |
| 0:45 | 5 · *(Optional)* Copernicus key | An on-demand product from the server |
| 0:52 | Wrap-up + where to go next | — |

**Slides:** a polished deck for the session is in [`slides/session.html`](slides/session.html) (open in a browser) and [`slides/session.pptx`](slides/session.pptx) (editable PowerPoint).

---

## 📚 Extra material — three deep-dive notebooks

After the session (or as homework), these go much deeper. Same one-click Colab badges.

| Module | Open in Colab | What it adds |
|--------|---------------|-------------|
| **1** — Satellite data | [![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/RicardoTeruelGutierrez/marmenor-eo-workshop/blob/main/notebooks/01_satellite_data_extraction.ipynb) | C2RCC vs L2A, Sentinel-3 SST, heatwave→oxygen, zone analysis |
| **2** — In-situ + ML | [![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/RicardoTeruelGutierrez/marmenor-eo-workshop/blob/main/notebooks/02_insitu_timeseries_ml.ipynb) | Buoy match-ups, honest cross-validation, anomaly early-warning |
| **3** — Build your own DB | [![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/RicardoTeruelGutierrez/marmenor-eo-workshop/blob/main/notebooks/03_build_database.ipynb) | Clean → SQLite + Parquet → a pollution study in SQL |

These three notebooks have no separate slides — they are self-explanatory, step by step. The polished deck is for the live session.
...
**The main session is the 1-hour live session** (notebook `00_live_session`, agenda
above, deck `slides/session.html`). For a longer course, the three deep-dive
notebooks extend it:

| Time | Module | Format |
|------|--------|--------|
| 60 min | **Live session** — discover, download, measure, detect, store | Notebook 00 + `slides/session.html` |
| +75 min | **Module 1** — Satellite data extraction (Sentinel-2 + Sentinel-3) | Notebook 01 |
| +70 min | **Module 2** — In-situ integration, time series, ML / anomalies | Notebook 02 |
...
├── notebooks/
│   ├── 00_live_session.ipynb                ← the 1-hour session (start here)
...
├── slides/
│   ├── session.html              ← live-session deck (open in a browser)
│   ├── session.pptx              ← same deck, editable PowerPoint
│   └── figs/                     ← real figures embedded in the deck
└── scripts/
    ...
    ├── build_session.py          ← rebuilds the live-session notebook
    ├── build_session_slides.py   ← rebuilds slides/session.html
    ├── build_session_pptx.py     ← rebuilds slides/session.pptx
...
*Synthetic datasets are calibrated against published literature but are not real measurements.*
